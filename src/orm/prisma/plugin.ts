...
export class PrismaPlugin implements Plugin {
  ...
  async install(app: VeloceTS): Promise<void> {
    ...
    console.log('✅ Prisma plugin installed successfully');
  }
  ...
  private async parseAndGenerateSchemas(): Promise<void> {
    try {
      if (!fs.existsSync(this.config.schemaPath!)) {
        console.warn(`⚠️  Prisma schema not found at ${this.config.schemaPath}`);
        return;
      }
      ...
      console.log(`📝 Generated Zod schemas for ${models.length} models`);
    } catch (error) {
