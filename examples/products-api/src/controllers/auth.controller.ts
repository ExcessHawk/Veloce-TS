...
export class AuthController {
  ...
  async register(@Body(RegisterSchema) body: z.infer<typeof RegisterSchema>) {
    ...
    const tokens = jwtProvider.generateTokens({ sub: id, username: body.username, email: body.email });

    return {
      success: true,
      user:    { id, username: body.username, email: body.email, roles: 'user' },
      tokens,
    };
  }
  ...
  async login(@Body(LoginSchema) body: z.infer<typeof LoginSchema>) {
    ...
    const tokens = jwtProvider.generateTokens({
